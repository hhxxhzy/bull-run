/**
 * 代码解析与模糊搜索（纯逻辑，无 vscode 依赖，可独立测试）
 * 支持输入：600519 / sh600519 / sz000001 / hk00700 / usAAPL / AAPL / 00700 / 1.600519(secid)
 */
import { StockItem } from './types';

export function parseInput(raw: string): StockItem | null {
  ...
  if (prefixMatch) {
    ...
    const marketMap: Record<string, number> = { sh: 1, sz: 0, bj: 0, hk: 116, us: 105 };
...
export function guessAMarket(code: string): number {
  if (/^(60|68|9|51|56|58)/.test(code)) { return 1; }
  return 0;
}
...
export async function searchStocks(keyword: string): Promise<SearchCandidate[]> {
  const url = `https://searchapi.eastmoney.com/api/suggest/get?input=${encodeURIComponent(keyword)}&type=14&count=12`;
  const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0' } });
  if (!res.ok) { throw new Error(`搜索接口异常 HTTP ${res.status}`); }
  ...
  for (const it of list) {
    ...
    if (!code || Number.isNaN(market)) { continue; }
    out.push({
      code,
      name: it.Name,
      secid: `${market}.${code}`,
