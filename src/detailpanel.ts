/**
 * 详情面板数据层：五档盘口 + 逐笔成交（东方财富公开接口）
 * 五档字段：f31/f32=卖5价/量 f33/f34=卖4 f35/f36=卖3 f37/f38=卖2 f39/f40=卖1
 *          f41/f42=买1价/量 f19/f20=买2 f17/f18=买3 f15/f16=买4 f13/f14=买5
 * 逐笔：details/get → "14:56:58,1272.98,7,7,1"（时间,价格,成交量(手),成交额,方向 1卖 2买）
 */
import { Quote } from './types';

export interface OrderLevel { price: number | null; volume: number | null }
...
export interface Tick { time: string; price: number; volume: number; bs: 1 | 2 }
...
/** 五档盘口（收盘后字段为空，做兜底） */
export async function fetchOrderBook(secid: string): Promise<OrderBook | null> {
  try {
...
/** 逐笔成交（最近 N 笔，pos=-N） */
export async function fetchTicks(secid: string, count = 30): Promise<Tick[]> {
  try {
    ...
    const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0' } });
    if (!res.ok) { return []; }
    const arr = ((await res.json()) as any)?.data?.details as string[] | undefined;
    if (!Array.isArray(arr)) { return []; }
    return arr.map((s): Tick => {
      const [time, p, v, , bs] = s.split(',');
      return { time, price: Number(p), volume: Number(v), bs: Number(bs) === 2 ? 2 as const : 1 as const };
...
export function panelChartSvg(
  points: { time: string; price: number; avg: number }[],
  preClose: number,
): string {
...
export function buildPanelHtml(
  q: Quote,
  trends: { points: { time: string; price: number; avg: number }[]; preClose: number } | null,
  ob: OrderBook | null,
  ticks: Tick[],
): string {
  ...
  const lvlRow = (l: OrderLevel, label: string) => {
